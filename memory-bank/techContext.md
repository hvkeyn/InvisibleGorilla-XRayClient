...
- On ALT Linux, apt-rpm may not provide the exact pinned .NET SDK patch and `$HOME/.dotnet` can be unwritable if an earlier run created it through root/sudo. `build.sh` must read `global.json`, resolve the matching executable into `DOTNET_CMD`, and install the Microsoft-script fallback into repo-local `.dotnet-sdk/` instead of `$HOME/.dotnet`. Installing only SDK 7 or ALT's `8.0.1xx` SDK is insufficient because repo-level `global.json` requests SDK `8.0.419`.
