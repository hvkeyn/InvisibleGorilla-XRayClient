...
- Validation after the Windows build fix: PowerShell parse check succeeds; `build.ps1 -Step DotNet` now reaches actual solution build on SDK `8.0.419`. Local validation only fails because a running `Invisible Gorilla XRay.exe` process locks the output exe, which is unrelated to SDK resolution.
