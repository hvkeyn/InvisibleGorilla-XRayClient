...
namespace InvisibleGorillaXRay.Services
{
    ...
    {
        // Hard-coded to the project's own release feed. Anyone forking the project can override the
        // owner/repo here to point the in-app updater at their own GitHub releases.
        ...
        public ReleaseAsset? PickAssetForCurrentPlatform(IReadOnlyList<ReleaseAsset> assets, string platform, string arch)
        {
            ...
            // First try exact platform + arch match; then platform-only; then return the first non-source asset.
            ReleaseAsset? best = null;
            foreach (ReleaseAsset asset in assets)
            {
                string name = asset.Name.ToLowerInvariant();
                bool platformMatch = name.Contains(platformLower);
                bool archMatch = name.Contains(archLower);
                if (platformMatch && archMatch)
                    return asset;
                if (platformMatch && best == null)
                    best = asset;
            }
