...
namespace InvisibleGorillaXRay.Services.Goida
{
    public static class GoidaActiveSelector
    {
        ...
        /// <summary>
        /// Picks the next node after a tunnel/VLESS failure. Walks the pool in order
        /// (round-robin) instead of always returning the fastest, so dead keys are
        /// skipped one-by-one until a working one is found.
        /// </summary>
        ...
        /// <summary>
        /// Auto-best failover: walk every enabled-list node in latency order
        /// (round-robin), accepting Unknown nodes too — TCP-only probes often mark
        /// dead VLESS keys as Ok, and restricting to Ok-only left failover stuck.
        /// </summary>
